...
export function buildReportPrompt(input: {
  ...
  return [
    ...
    'The briefing investments snapshot is deterministic evidence. Publish investment changes only when materialChanges marks them material. For changed positions with a non-zero baseline, show signed currency delta and percent together. Use added or removed wording without inventing a percentage. Do not add generic portfolio summaries or copy investment statistics into durable memory.',
    'Describe one economic investment change once. Do not repeat the same movement at total, type, subtype, and code levels unless a lower level adds distinct material context.',
...
export function buildReviewerInstructions(): string {
  return [
    ...
    'Audit investment wording against the briefing snapshot. Keep only investment changes marked material. Use added or removed wording without a percentage, and do not turn deterministic allocation data into durable memory.',
    'Remove duplicated investment change claims that merely restate the same economic movement at several allocation levels.',
